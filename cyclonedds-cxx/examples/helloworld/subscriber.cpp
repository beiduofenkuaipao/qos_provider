 ...
 * terms of the Eclipse sublic License v. 2.0 which is available at
...
int main() {
    try {
        ...
        dds::core::QosProvider providersub("file:///home/dong/work/10_cyc_qos_test_4/cyclonedds-cxx/examples/helloworld/test_qos.xml","BarQosProfile");
        dds::domain::DomainParticipant participant(domain::default_id(),providersub.participant_qos());
        dds::topic::Topic<HelloWorldData::Msg> topic(participant, "HelloWorldData_Msg",providersub.topic_qos());
        dds::sub::Subscriber subscriber(participant, providersub.subscriber_qos());
        dds::sub::DataReader<HelloWorldData::Msg> reader(subscriber, topic,providersub.datareader_qos("::BarQosProfile::TransientKeepLast"));

#else
        dds::domain::DomainParticipant participant(domain::default_id());
        dds::topic::Topic<HelloWorldData::Msg> topic(participant, "HelloWorldData_Msg");
        dds::sub::qos::SubscriberQos sqos;
        sqos.policy(dds::core::policy::Partition("Hellow example"));
        ...
        dds::sub::DataReader<HelloWorldData::Msg> reader(subscriber, topic, rqos);
