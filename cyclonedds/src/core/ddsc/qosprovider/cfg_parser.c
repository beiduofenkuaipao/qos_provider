...
static int elem_open_cb(void *varg, uintptr_t parentinfo, uintptr_t *eleminfo, const char *name)
{
    ...
    *eleminfo = (uintptr_t) e;
    parg->stack = c_iterInsert(parg->stack, ddsrt_strdup(name));
    return 0;
}

static int elem_attr_cb(void *varg, uintptr_t eleminfo, const char *name, const char *value)
{
    cf_element e = (cf_element) eleminfo;
    c_string xvalue = ddsrt_expand_envvars(value,0);
    OS_UNUSED_ARG(varg);
...
static int elem_data_cb(void *varg, uintptr_t eleminfo, const char *data)
{
    cf_element e = (cf_element) eleminfo;
    c_string xdata = ddsrt_expand_envvars(data,0);
    OS_UNUSED_ARG(varg);
...
static int elem_close_cb(void *varg, uintptr_t eleminfo)
{
    struct parg *parg = varg;
    OS_UNUSED_ARG(eleminfo);
    ddsrt_free(c_iterTakeFirst(parg->stack));
    return 0;
}
...
static void init_cb_arg(struct ddsrt_xmlp_callbacks *cb, struct parg *arg, cf_element *spliceElement)
{
    *spliceElement = NULL;
...
cfgprs_status cfg_parse_ospl(const char *uristr, cf_element *spliceElement)
{
    ...
    if ((st = ddsrt_xmlp_new_string(cfg->ptr, &parg, &cb)) == NULL) {
